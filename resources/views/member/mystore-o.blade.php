...
<head>
    ...
    <style>
        ...
        .order-tab-item:hover { color: white; }
...
<body class="dashboard-body">
        ...
        <div class="main-content">
            ...
            <div class="card">
                ...
                <div class="order-tabs">
                    <a href="{{ route('member.orders.index', ['status' => 'pesanan-diproses']) }}" 
                       class="order-tab-item {{ $status == 'pesanan-diproses' ? 'active' : '' }}">
                       <i class="fa-regular fa-clock"></i> Perlu Diproses
                    </a>
                    <a href="{{ route('member.orders.index', ['status' => 'pesanan-diterima']) }}" 
                       class="order-tab-item {{ $status == 'pesanan-diterima' ? 'active' : '' }}">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td width="40%">
                                    ...
                                    <div style="display: flex; gap: 10px; margin-bottom: 10px;">
                                        ...
                                        <div>
                                            <div style="color: white; font-size: 13px; font-weight: 500;">{{ $detail->product->name }}</div>
                                            <div style="color: #9ca3af; font-size: 11px;">{{ $detail->qty }} x Rp {{ number_format($detail->product->price,0,',','.') }}</div>
                                        </div>
                                    </div>
                                    @endforeach
                                ...
                                <td width="30%">
                                    <div style="color: white; font-weight: 600; font-size: 13px;">{{ $order->user->name }}</div>
                                ...
                                <td width="15%">
                                    @if($order->shipping == 'pesanan-diproses')
                                        <span class="badge bg-pending" style="font-size: 11px;">Menunggu Proses</span>
                                    @elseif($order->shipping == 'pesanan-diterima')
                                        <span class="badge bg-success" style="font-size: 11px;">Selesai/Diterima</span>
                                    @endif
                                </td>

                                <td width="15%">
                                    @if($order->shipping == 'pesanan-diproses')
                                        <button onclick="openModal('{{ $order->id }}')" class="btn-sm" style="background: #6366f1; width: 100%; margin-bottom: 5px; padding: 8px 0;">
                                            <i class="fa-solid fa-paper-plane"></i> Proses
                                        </button>
                                    @else
                                        <button class="btn-sm" style="background: #111827; border: 1px solid #374151; color: #4ade80; width: 100%; cursor: default;">
                            ...
                            <tr>
                                <td colspan="4" style="text-align: center; padding: 40px;">
                                    <i class="fa-solid fa-folder-open" style="font-size: 40px; color: #374151; margin-bottom: 10px;"></i>
        ...
        <div class="modal-box">
            ...
            <form id="processForm" method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="shipping_type" class="form-input" placeholder="Contoh: JNE / Kurir Toko" required>
                </div>
                ...
                <div style="display: flex; gap: 10px; margin-top: 25px;">
                    <button type="button" onclick="closeModal()" class="btn-outline" style="flex: 1; border-color: #374151; color: #d1d5db;">Batal</button>
                    <button type="submit" class="submit-btn" style="flex: 1; background: #6366f1; padding: 10px; font-size: 14px;">Selesaikan Pesanan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function openModal(orderId) {
            ...
            form.action = "{{ route('member.orders.update', ':id') }}".replace(':id', orderId);
