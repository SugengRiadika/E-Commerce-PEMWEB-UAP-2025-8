   ...
   <nav class="navbar">
    ...
    <div class="seller-container">
        ...
        <main class="content-center">
            <div class="section-header page-title"><h3>Ringkasan Penjualan</h3></div>
            <div class="stats-grid">
                {{-- KARTU SALDO --}}
                <div class="stat-card">
                    <div class="stat-info">
                        <h4>Saldo Toko Aktif</h4>
                        <div class="num" style="color: #4ade80;">Rp {{ number_format($currentBalance, 0, ',', '.') }}</div>
                        <a href="#" style="font-size: 12px; color: #6366f1; text-decoration: none;">Tarik Dana &rarr;</a>
                    </div>
                ...
                <div class="stat-card">
                    <div class="stat-info">
                        ...
                        <a href="#" style="font-size: 12px; color: #6366f1; text-decoration: none;">Tambah Produk &rarr;</a>
                    </div>
            ...
            <div class="card">
                <div class="card-header-sm" style="display: flex; justify-content: space-between; align-items: center;">
                    <h3>Pesanan Terbaru (Perlu Diproses)</h3>
                    <a href="#" class="btn-outline" style="font-size: 12px;">Lihat Semua</a>
                </div>
                <table class="data-table">
                    <thead>
                        <tr><th>ID Pesanan</th><th>Produk</th><th>Total</th><th>Status</th><th>Aksi</th></tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td>#{{ $order->id }}</td>
                            <td>
                                {{-- Menampilkan nama produk pertama sebagai preview --}}
                                @if($order->details && $order->details->isNotEmpty())
                                    <div>{{ $order->details->first()->product->name }}</div>
                                    @if($order->details->count() > 1)
                                        <small style="color: #9ca3af;">+ {{ $order->details->count() - 1 }} lainnya</small>
                                    @endif
                                @else
                                    <div>Data Produk Tidak Tersedia</div>
                                @endif
                            </td>
                            ...
                            <td>
                                <div class="action-btn-group">
                                    <button class="btn-sm btn-view">Detail</button>
                                    <button class="btn-sm btn-edit">Input Resi</button>
                                </div>
                            </td>
                        </tr>
                        @empty
